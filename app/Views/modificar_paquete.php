    ...
    <div class="contenedor">
        ...
        <main>
            ...
            <section class="formulario-paquete">
                ...
                <form action="<?= base_url('actualizar_paquete/' . $p['id']) ?>" method="post" enctype="multipart/form-data">

                    <div class="form-contenedor">
                        <div>
                            <label for="destino">Destino:</label>
                            <input type="text" id="destino" name="destino" value="<?= $p['destino'] ?>" required>

                            <label for="hotel">Hotel:</label>
                            <input type="text" id="hotel" name="hotel" value="<?= $p['hotel'] ?>" required>

                            <label for="dias">Días:</label>
                            <input type="number" id="dias" name="dias" value="<?= $p['dias'] ?>" min="1" required>

                            <label for="stock">Stock:</label>
                            <input type="number" id="stock" name="stock" value="<?= $p['stock'] ?>" min="0" required>
                        </div>

                        <div>
                            <label for="precio">Precio:</label>
                            <input type="number" step="0.01" id="precio" name="precio" value="<?= $p['precio'] ?>" min="0" required>

                            <label for="categoria">Categoría:</label>
                            <select id="categoria" name="categoria" required>
                                <option value="">-- Seleccioná una categoría --</option>
                                <option value="Turismo" <?= ($p['categoria'] == 'Turismo') ? 'selected' : '' ?>>Turismo</option>
                                <option value="Playas" <?= ($p['categoria'] == 'Playas') ? 'selected' : '' ?>>Playas</option>
                                <option value="Aventura" <?= ($p['categoria'] == 'Aventura') ? 'selected' : '' ?>>Aventura</option>
                            </select>
                            ...
                            <input type="number" id="descuento" name="descuento" value="<?= $p['descuento'] ?>" min="0" max="100" step="0.01">
                        </div>
                    </div>

                    <label for="descripcion_hotel">Descripción del Hotel:</label>
                    <textarea id="descripcion_hotel" name="descripcion_hotel"><?= $p['descripcion_hotel'] ?></textarea>

                    <label for="descripcion_paquete">Descripción del Paquete:</label>
                    <textarea id="descripcion_paquete" name="descripcion_paquete"><?= $p['descripcion_paquete'] ?></textarea>

                    <fieldset>
                        <legend>Incluye:</legend>
                        <input type="checkbox" id="incluye_vuelo" name="incluye_vuelo" value="1" <?= $p['incluye_vuelo'] ? 'checked' : '' ?>>
                        <label for="incluye_vuelo">Vuelo</label><br>

                        <input type="checkbox" id="incluye_traslado" name="incluye_traslado" value="1" <?= $p['incluye_traslado'] ? 'checked' : '' ?>>
                        <label for="incluye_traslado">Traslado</label><br>

                        <input type="checkbox" id="incluye_hotel" name="incluye_hotel" value="1" <?= $p['incluye_hotel'] ? 'checked' : '' ?>>
                        <label for="incluye_hotel">Hotel</label><br>

                        <input type="checkbox" id="incluye_excursion" name="incluye_excursion" value="1" <?= $p['incluye_excursion'] ? 'checked' : '' ?>>
